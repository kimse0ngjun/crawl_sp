...
def get_addtional_info_ssd(driver, url):
    ...
    try:
        ...
        skip_sections = [
            "등록년월", "AURA SYNC", "MYSTIC LIGHT",
            "RGB FUSION", "POLYCHROME", "KC",
            "적합성평가인증", "안전확인인증",
        ]
        ...
        print("get_addtional_info_ssd error:", e)
        return {}
...
def crawl_danana_ssd():
    ...
    driver.quit()
    save_to_json(results, "ssd_danawa.json")
    return results

def save_to_json(results, filename="ssd_danawa.json"):
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=4)


if __name__ == "__main__":
    results = crawl_danana_ssd()
    save_to_json(results, "ssd_danawa.json")
    print("JSON 저장 완료")
